        ...
        (error, results) => {
            ...
            if (results.rowCount > 0) {
                ...
                return res.status(401).json({
                    code: 401,
                    message: "User does not exsist! Please try with another user!",
                });
            }
        }
    );
...
router.post("/login", (req, res, next) => {
    ...
    pool.query(
        "SELECT A.*,B.id as role_id, B.name as role_name, B.page FROM users as A join role as B on A.role=B.id WHERE email = $1 ", [email],
        (error, results) => {
            if (error) {
                throw error;
            }
            ...
            if (results.rowCount > 0) {
                ...
                bcrypt.compare(password, results.rows[0].password, function(
                    ...
                    if (result_hash == true) {
                        ...
                        // console.log("Role: ", results.rows[0]);
                        ...
                        return res.status(200).json({
                            ...
                            role: results.rows[0].page,
                            rolename: results.rows[0].role_name,
                        });
                    }
                    // wrong password
                    return res.status(401).json({
                ...
                return res.status(401).json({
                    code: 401,
                    message: "User does not exsist! Please signup for registration",
                });
            }
        }
    );
